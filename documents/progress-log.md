...
- Consolidated repository structure around `pages/` and aligned documentation paths.
- Added root and layered agent guidance (`AGENTS.md`, `pages/AGENTS.md`, `templates/AGENTS.md`).
- Added Cursor hard constraints/content rules and enabled project-scoped skills.
- Finalized initial product positioning updates in public-facing docs/pages.
- Established durable workflow baseline and continuity rules in repository docs.

### Week of 2026-04-17

- Drafted and published Story Pack v1 spec, schema, and minimal runnable sample.
- Added runtime loader entrypoint and migrated validation flow to runtime-centric APIs.
- Implemented cross-reference validation for core IDs and expanded validator tests.
- Added event trigger selection logic and runtime condition operator coverage.
- Updated README/index pages to expose story-pack assets and usage entrypoints.

### Week of 2026-04-24

- Extended runtime with encounter entry checks and scene-aware validation behavior.
- Added deterministic retrieval foundation (`rules_index/` chunking, storage, entities, router, eval).
- Added retrieval build/import/eval scripts and benchmark fixtures for regression checks.
- Introduced retrieval policy/ADR docs for source honesty, pinning, and staged embedding decisions.
- Expanded automated test matrix across story-pack runtime and retrieval modules.

### Week of 2026-05-01

- Validated against real upstream corpora snapshots using pinned commit references.
- Hardened importer behavior for real dataset shapes and added inspection tooling.
- Updated repository safety guidance for `vendor/` and `build/` staging hygiene.
- Added and audited web-first MVP execution plan, then hardened API contract and delivery gates.
- Applied log/plan hygiene updates and aligned release-readiness expectations (including CI/CD minimums).

## 2026-04-10 (Workflow and structure baseline)
...
## 2026-04-12 (Story-Pack v1 Execution)
...
## 2026-04-14 (Cross-Reference Validator)
...
## 2026-04-17 (Runtime Entry Implementation)
...
## 2026-04-19 (Event Trigger Selector)
...
## 2026-04-21 (Story-pack validation, encounter entry, docs)
...
## 2026-04-24 (Dual-layer rules retrieval foundation)
...
## 2026-04-26 (Real upstream data validation)
...
## 2026-04-28 (Session handoff hygiene)
...
## 2026-05-01 (Web-first MVP planning)

### Completed

- Performed repository-wide status review focused on “browser-usable MVP without standalone graphical client”.
- Added executable plan document:
  - `documents/mvp-plan-web-first.md`
- Defined MVP scope boundaries:
  - In-scope: web text loop, minimal API session/turn flow, story-pack runtime integration, retrieval source transparency
  - Out-of-scope: voice, 2D tactical visualization, embedding/reranker
- Broke delivery into staged milestones (scope freeze -> API skeleton -> turn orchestration -> retrieval integration -> web page -> release readiness).

### Decisions

- Treat web-first text interaction as the primary MVP target for near-term execution.
- Prioritize stable gameplay loop and source-traceable responses before multimodal or graphical enhancements.

### Next

- Start implementation from phase 1 in `documents/mvp-plan-web-first.md`:
  1) create minimal API skeleton
  2) add turn orchestration endpoint
  3) connect a playable web page in `pages/`
...
## 2026-05-02 (MVP Plan Audit)

### Completed (MVP Plan Audit)

- Audited `documents/mvp-plan-web-first.md` against repository constraints in:
  - `AGENTS.md`
  - `.cursor/rules/agent-hard-constraints.mdc`
  - `documents/rules-data-sources.md`
- Produced prioritized findings on execution risk, API contract clarity, and repository-rule alignment.

### Decisions (MVP Plan Audit)

- Treat `documents/mvp-plan-web-first.md` as execution-ready only after adding concrete API contract details, deployment assumptions, and AGENTS.md update requirements for new directories.

### Next (MVP Plan Audit)

- Apply audit fixes to `documents/mvp-plan-web-first.md` in a follow-up edit pass if approved.

## 2026-05-03 (MVP Plan Hardening)

### Completed (MVP Plan Hardening)

- Updated `documents/mvp-plan-web-first.md` to make the plan execution-ready:
  - added frozen API contract section (request/response/error structure)
  - added session storage strategy split (`memory` for local, `sqlite` for demo/prod)
  - added explicit governance step to update `AGENTS.md` and `README.md` when introducing `app/`
  - added minimum CI/CD pipeline requirements and acceptance criteria
  - reordered immediate execution sequence to include retrieval integration before web demo closure

### Decisions (MVP Plan Hardening)

- Keep API contract frozen at the end of phase 1 and allow only backward-compatible additions after that point.
- Treat MVP as demo-ready only after phase 5 CI/CD and release checks are complete.

### Next (MVP Plan Hardening)

- Start implementation from phase 1 with contract-first API work and dual session store interface.

## 2026-05-03 (Date Redaction for Logs/Plans)

### Completed (Date Redaction for Logs/Plans)

- Removed concrete date heading from `documents/progress-log.md`.
- Replaced progress log template date placeholder with a non-date session heading.
- Removed concrete ADR date field from `documents/rules-retrieval-embedding-adr.md`.

### Decisions (Date Redaction for Logs/Plans)

- Keep logs and plans free of explicit calendar dates unless user explicitly requests dated records.

### Next (Date Redaction for Logs/Plans)

- Continue using session-title style entries in project logs.
